...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--repo PATH] [additional claude args...]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repo)
      [[ $# -lt 2 ]] && { echo >&2 "[forge-cli] --repo needs a path"; usage; exit 1; }
      TARGET_REPO="$(cd "$2" && pwd)"
      shift 2
      ;;
    --agent)
      [[ $# -lt 2 ]] && { echo >&2 "[forge-cli] --agent needs a profile name"; usage; exit 1; }
      AGENT_PROFILE="$2"
      shift 2
      ;;
    -h|--help)
...
if [[ "${FORGE_NO_SYNC:-0}" != "1" ]]; then
  echo "[forge-cli] Resyncing Forge repo at ${FORGE_ROOT}..."
...
PLUGIN_DIRS=(
  ...
  "${FORGE_ROOT}/plugins/prompt-forge"
)
...
if [[ -n "${TARGET_REPO}" ]]; then
  echo "[forge-cli] Working directory: ${TARGET_REPO}"
  cd "${TARGET_REPO}"
fi
...
if [[ -n "${FORGE_AGENT_PROFILE:-}" ]]; then
  echo "[forge-cli] Agent profile: ${FORGE_AGENT_PROFILE}"
fi

echo "[forge-cli] Launching (pwd=$(pwd)): ${CMD[*]}"
exec "${CMD[@]}"
