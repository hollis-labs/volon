...
Copies the current Forge repo into a sanitized Volon template suitable for
...
if [[ "${TARGET_PATH}" == "${FORGE_ROOT}" ]]; then
  echo "[volon] Target cannot be the Forge repo itself" >&2
  exit 1
fi
...
echo "[volon] Copying Forge repo to ${TARGET_PATH}..."
...
[volon] Next steps:
  1. cd ${TARGET_PATH}
  2. scripts/forge-cli.sh /bootstrap-update
