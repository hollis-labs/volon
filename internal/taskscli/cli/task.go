...
import (
	...
	"github.com/hollis-labs/forge/internal/taskscli/taskfile"
)
...
// Run is the entry point for `forge task`.
...
func (r *Runner) Dispatch(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: forge task <subcommand> [flags]")
	}
...
func (r *Runner) cmdCreate(args []string) error {
	fs := flag.NewFlagSet("forge task create", flag.ContinueOnError)
...
func (r *Runner) cmdStart(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: forge task start <id>")
	}
...
func (r *Runner) cmdDone(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: forge task done <id>")
	}
...
func (r *Runner) cmdList(args []string) error {
	fs := flag.NewFlagSet("forge task list", flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	var statuses, types, tags, priorities csvList
...
func (r *Runner) updateIndexFromFile(tf *taskfile.File) {
	...
	if err != nil {
		r.warn("SQLite unavailable (%v); run `forge task reindex` once resolved", err)
		return
	}
	defer store.Close()
...
func (r *Runner) warn(format string, args ...interface{}) {
	fmt.Fprintf(r.stderr, "[forge task] %s\n", fmt.Sprintf(format, args...))
}
...
func (r *Runner) printRows(rows []index.TaskRow) {
	fmt.Fprintf(r.stdout, "%-16s %-6s %-3s %-10s %s\n", "ID", "Status", "Pri", "Updated", "Title")
	for _, row := range rows {
		...
		fmt.Fprintf(r.stdout, "%-16s %-6s %-3s %-10s %s\n",
			row.ID, row.Status, row.Priority, updated, row.Title)
	}
}
