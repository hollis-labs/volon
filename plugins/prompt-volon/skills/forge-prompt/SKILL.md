---
name: volon-prompt
aliases: ["prompt-volon", "make-prompt"]
description: Generate a copy/paste-ready Volon prompt from a natural language intent. Infers the correct template (inception/task/ideation/planning/patch) and fills all guardrails automatically.
...
Generate a single, copy/paste-ready Volon prompt from a natural language intent description.
...
**Aliases:** `/prompt-volon`, `/make-prompt` — all invoke this skill identically.
...
If `$0` is absent: output `ERROR: intent is required. Usage: /volon-prompt "<intent>"` and stop.
...
Output one line: `[volon-prompt] mode inferred: <MODE>`
...
| `{{REPO}}` | REPO |
| `{{DATE}}` | DATE |
| `{{CONSTRAINTS}}` | CONSTRAINTS if non-empty, else `"(none — use Volon defaults)"` |
...
`[volon-prompt] done — <MODE> prompt generated for: <INTENT>`
