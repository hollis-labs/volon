...
- `git.branch_prefix` → default: `volon/`
...
output `ERROR: worktrees disabled in volon.yaml. Create branch manually: git checkout -b <branch_prefix><slug>` and stop. **(E2)**
...
Construct:
- `branch_name` = `<branch_prefix><slug>` (e.g., `volon/worktree-start`)
...
- Never hardcode `.worktrees/` or `volon/` — always read from config.
...
- All config values read from `volon.yaml` in Step 1 — no defaults embedded in commands.
