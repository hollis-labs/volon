...
or print manual steps if not. Reads all policy from `forge.yaml`.
...
- `git.pr.title_prefix` → default: `forge:`
- `git.pr.body_template` → default: `.forge/templates/pr-body.md`

If `git.pr_mode` is `off`:
output `PR creation is disabled (git.pr_mode: off in forge.yaml).` and stop.
...
Check if `current_branch` starts with `git.branch_prefix` (e.g., `forge/`).

If it does not:
output `WARN: current branch "<current_branch>" does not match prefix "<branch_prefix>". Proceeding anyway.`
(Do not stop — the user may be opening a PR from a non-Forge branch intentionally.)
...
- Else: output `ERROR: cannot determine PR base branch — set git.pr.base_branch in forge.yaml.` and stop.
...
- All config values read from `forge.yaml` in Step 1 — no hardcoded defaults in commands.
