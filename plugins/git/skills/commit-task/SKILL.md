...
- `mode` (optional): `iteration` or `isolated`. If omitted, reads `git.commit_mode` from `volon.yaml` (default: `iteration`).
...
If `task-id` provided: read `.volon/tasks/<task-id>.md`.
Otherwise: find the most recently updated task with `status: done` in `.volon/tasks/`.
...
**Iteration mode:**
Run: !`git commit -m "volon: iteration <N> — <task-count> tasks (<task-id>)"`

Where:
- `<N>` = current iteration from `.volon/bootstrap.md`
- `<task-count>` = number of tasks completed this iteration
- `<task-id>` = this task's ID (or comma-separated list if batching)

**Isolated mode:**
Run: !`git commit -m "volon: <task-id> — <task-title>"`
