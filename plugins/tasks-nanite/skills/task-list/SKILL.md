...
List tasks from the backend defined by `storage.backend` in `forge.yaml`.
...
Build search query from filters. Forge tasks are tagged with `<tag_prefix>task`.
...
by checking each item's tags list (tags are prefixed: `forge/priority:A`, `forge/context:dev`, etc.).
