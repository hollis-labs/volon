...
- `workflows.update_docs.enabled` → if `false`: output `workflow-docs-review is disabled in volon.yaml.` and stop.
...
- All config read from `volon.yaml` in Step 1 — no hardcoded paths or modes.
