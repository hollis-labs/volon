...
description: Guided workflow for creating a new Volon skill — ideation, requirements, spec, plan, tasks.
...
- `workflows.new_skill.enabled` → if `false`: output `workflow-new-skill is disabled in volon.yaml.` and stop.
...
Read `.volon/pcc/01_architecture.md` to understand existing plugin structure.
