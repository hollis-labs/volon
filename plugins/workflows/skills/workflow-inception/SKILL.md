...
description: Run the Volon inception loop — read bootstrap, select next work unit, execute, verify, log, finalize. The canonical self-building loop.
...
The "system builds itself" loop. Volon uses its own artifacts (tasks, PCC, bootstrap, skills, workflows) to evolve a repo with high throughput and low drift.
...
2. Read `.volon/bootstrap.md` (if present) — note iteration number, next actions, blockers.
3. Read `.volon/pcc/00_project.md` (high-level only) — note version, stage, goals.
4. Read `.volon/tasks/TASK-*.md` files — collect status counts and top-3 `todo` tasks by priority.
...
1. Run `/bootstrap-update` to regenerate `.volon/bootstrap.md` from repo ground truth.
