...
description: Guided workflow for defining a new Volon agent role — purpose, scope, context, interface, plan, tasks.
...
- `workflows.new_agent.enabled` → if `false`: output `workflow-new-agent is disabled in volon.yaml.` and stop.
...
Read `.volon/agent-boot.md` and `.volon/boot/` role addenda for current role definitions.
