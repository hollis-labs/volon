...
- `workflows.app_investigation.enabled` → if `false`: output `workflow-app-investigation is disabled in forge.yaml.` and stop.
...
Read the updated `.forge/pcc/` after refresh.
