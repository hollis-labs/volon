...
description: Guided workflow for creating a new Forge plugin — ideation, requirements, spec, plan, tasks.
...
- `workflows.new_plugin.enabled` → if `false`: output `workflow-new-plugin is disabled in forge.yaml.` and stop.
