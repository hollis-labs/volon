...
**Phase E — Backlog tasks** — requires Phase D (only if `track_tasks: true` in forge.yaml)

Create TASK files in `.forge/tasks/` for each implementation task from the plan.
...
- Backlog tasks if plan produced (per forge.yaml `track_tasks` setting)
