...
1. Run project lint/test if available:
   - Check `forge.yaml` for quality config (`quality.modes`).
...
1. Write run log at `.forge/logs/run-{{DATE}}.md`:
   ```
   # Run Log — Patch — {{DATE}}
   ...
   - <what to do next>
   ```
2. Stage only the patched files (not `.forge/` state files unless they were in the patch):
   ```
