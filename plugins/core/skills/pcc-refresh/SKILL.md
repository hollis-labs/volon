...
Update **L0 Global PCC** (`.volon/pcc/global/*`) with **minimal diffs** based on repo reality.
Touch only affected sections. Never invent content.

> **Note:** This skill refreshes L0 (Global) PCC only.
> L2 Task PCC capsules (`.volon/pcc/tasks/`) are managed by `/pause-task` and `/resume-task`.
...
- Derive `global_path` = `<pcc.location>/<pcc.global_dir>` → default: `.volon/pcc/global`
...
All PCC files in this table live under `<global_path>/` (default: `.volon/pcc/global/`).

| Path pattern | Affects PCC file(s) |
|---|---|
| `README.md` | `00_project.md` |
| `volon.yaml` | `00_project.md`, `01_architecture.md` |
| `.volon/volon.yaml` | `00_project.md`, `01_architecture.md` |
...
### 00_project.md
Update when: `README.md`, `volon.yaml`, or `docs/01_config.md` changed.
- Refresh Active Config block if `volon.yaml` changed.
- Refresh Goals/Non-goals if `README.md` changed.
- Do not modify Identity block unless project name or version changed.

### 01_architecture.md
Update when: any `plugins/**`, `volon.yaml`, or `docs/02_pcc.md` changed.
- Update only the row(s) in the plugin/skill table that correspond to changed plugins.
- Update storage or data flow sections only if `volon.yaml` changed.
...
### 04_backlog.md
Update when: `artifacts/**`, `.volon/tasks/**`, or any unmatched changed path.
