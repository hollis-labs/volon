...
Write (or overwrite) `.volon/pcc/tasks/<task_id>.md` using the schema from
`.volon/templates/task-pcc-capsule.md`.
...
### Step 4 — Update `.volon/bootstrap.md`
...
- Append a pause entry to the current run log OR create a minimal pause log in `.volon/logs/`.
...
List files changed:
- `.volon/tasks/<task_id>.md` (status → paused, Updates appended)
- `.volon/pcc/tasks/<task_id>.md` (Task PCC capsule written)
- `.volon/bootstrap.md` (paused_task_id, paused_at, resume_hint updated)
- `.volon/logs/<run-log>.md` (if observability enabled)
