...
Read `.forge/bootstrap.md` and determine the task to resume:
...
Read in this order:
1. `forge.yaml` — config (pcc.location, pcc.global_dir, pcc.tasks_dir)
2. `.forge/pcc/global/` — L0 Global PCC (project-wide context)
3. `.forge/pcc/tasks/<task_id>.md` — L2 Task PCC capsule (if exists; see Step 3)
4. `.forge/tasks/<task_id>.md` — authoritative task file (source of truth)
5. `.forge/logs/` — recent run log (last entry)

### Step 3 — Load Task PCC capsule (L2)

Attempt to read `.forge/pcc/tasks/<task_id>.md`.
