...
description: Generate/update .volon/bootstrap.md from repo ground truth (tasks/logs/PCC/config).
...
- `.volon/bootstrap/history/bootstrap-iteration-<N>.md`
...
Extract and note:
- `bootstrap.path` → default: `.volon/bootstrap.md`
- `bootstrap.history_path` → default: `.volon/bootstrap/history`
...
- **Config:** `volon.yaml` for workflows, quality modes, and storage settings
...
1. **Header**: `# Volon Bootstrap`, Iteration N, Generated date, source of truth note.
...
Check that `.volon/agent-boot.md` exists.
- If it exists: note it in the output ("boot pack present").
- If it does NOT exist: warn "WARN: .volon/agent-boot.md missing — agents may not boot correctly. Run task to create it."

Also check `.volon/boot/` contains orchestrator.md, worker.md, reviewer.md.
...
- List changed files.
- Print full contents of `.volon/bootstrap.md`.
