...
- `profile-name` (required): slug referencing `.forge/agents/<slug>.yaml`
...
PROFILE_FILE=".forge/agents/${PROFILE_SLUG}.yaml"
```
...
if [[ ! -f "$PROFILE_FILE" ]]; then
  ...
  ls .forge/agents/*.yaml 2>/dev/null | sed 's#.*/##; s/\.yaml$//'
  exit 1
fi
```
...
4. Remind yourself (and the user) of the next actions:
   ...
   - To launch a CLI session directly in this profile, run `FORGE_AGENT_PROFILE=$PROFILE_SLUG scripts/forge-cli.sh --repo <path>` or pass `--agent $PROFILE_SLUG`.
