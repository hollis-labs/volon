...
Capture ideas and work items to `.volon/backlog/` without committing to execution.
...
1. Determine next BACKLOG ID:
   - Run: !`ls .volon/backlog/BACKLOG-*.md 2>/dev/null | sort | tail -1 || true`
...
2. Write `.volon/backlog/BACKLOG-YYYYMMDD-NNN.md`:
...
project: <project from volon.yaml>
tags: [<tags>]
...
Title: <title>
File: .volon/backlog/BACKLOG-YYYYMMDD-NNN.md
...
Run: !`ls .volon/backlog/BACKLOG-*.md 2>/dev/null || echo "NO_BACKLOG"`
...
1. Read `.volon/backlog/<backlog-id>.md`. Fail fast if not found.
...
- Backlog items live in `.volon/backlog/` — never in `.volon/tasks/`.
...
- If `.volon/backlog/` does not exist: create it before writing the first item.
