...
- Valid domains: `user`, `forge`. If invalid: output `ERROR: domain must be user or forge.` and stop.
...
If `MISSING`: output `ERROR: workflows/<domain>/ directory not found. Ensure forge layout is initialized.` and stop.
...
- Set `intent`: `user_workflow` if `domain=user`, `forge_workflow` if `domain=forge`
...
- `domain: forge` is allowed but unusual — prefer `domain: user` for new workflows.
