---
id: ka-2026-02-22-forge-prompt-goldens
type: knowledge_artifact
intent: knowledge_artifact
status: complete
project: forge
tags: [forge-prompt, testing, fixtures, goldens]
created_at: 2026-02-22
updated_at: 2026-02-22
---

# forge-prompt — Golden Fixtures

Five reference examples showing `/forge-prompt` input → inferred mode → generated output.
Use these to verify the prompt generator produces correct Forge-style prompts.

Each golden is shown exactly as the skill would output it (filled template, no raw placeholders).

---

## Golden 1 — Inception: install forge and run loop for new app

**Command:**
```
/forge-prompt "install forge into nanite and run inception for chat addon"
```

**Inferred mode:** `inception` (intent contains "install forge" and "run inception")
...
**Intent:** install forge into nanite and run inception for chat addon
...
2. Read `.forge/bootstrap.md` — note iteration number, next actions, blockers.
3. Read `.forge/pcc/` (high-level only — `00_project.md` if present).
4. Read `.forge/tasks/TASK-*.md` — collect status counts and top-3 `todo` tasks (A>B>C, oldest first).
...
3. If no `todo` tasks → promote a backlog item from `.forge/backlog/`.
...
Write a run log entry at `.forge/logs/run-2026-02-22-iter<N>.md`.
...
**Command:**
```
/forge-prompt "add layered PCC L0/L2 and wire pause/resume"
```
...
---PROMPT-START---
You are operating in **forge** in **Orchestrator Mode**.
...
2. Read `.forge/bootstrap.md` — identify the task to execute.
3. Read the target task file in `.forge/tasks/`.
...
Write run log at `.forge/logs/run-2026-02-22.md`.
...
**Command:**
```
/forge-prompt "run ideation to brainstorm names with vibe constraints" \
...
**Command:**
```
/forge-prompt "create a PRD and spec for a new sprint-based workflow" \
  --mode planning \
...
- All artifact frontmatter must use Forge schema (see `docs/03_workflow-contracts.md`).
...
1. Run: `cat forge.yaml 2>/dev/null || echo "NO_CONFIG"`
2. Read `.forge/pcc/` (global context — goals, architecture, constraints).
...
- **Phase E — Backlog tasks:** `.forge/tasks/TASK-*.md` (requires Phase D, if `track_tasks: true`)
...
- No application code changes during this session.
- All artifacts must use Forge frontmatter schema.
...
**Command:**
```
/forge-prompt "apply the patch from review-fixes.patch and verify no data loss"
```
...
Do not rely on prior chat context. Repo artifacts are the only truth:
- `forge.yaml`, `.forge/bootstrap.md`, `git status`
...
1. Run lint/test if available (check `forge.yaml` quality config).
...
1. Write run log at `.forge/logs/run-2026-02-22.md`.
