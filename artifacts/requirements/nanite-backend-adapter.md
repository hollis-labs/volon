...
The Nanite backend adapter adds an alternative task storage path to forge so
that task operations are routed to the user's Nanite instance when
`storage.backend: nanite` is set in `forge.yaml`. The file backend must
...
1. `forge.yaml` accepts `storage.backend: nanite` without error when the adapter is installed.
...
3. `/task-list` with `storage.backend: nanite` returns only forge-tagged Nanite items,
...
- Nanite connection config lives under `storage.nanite` in `forge.yaml`
  (e.g., `vault`, `api_url`, `tag_prefix`).
- Task IDs in Nanite mode follow Nanite's native ID scheme; the forge canonical
  ID is stored as a tag or in the item body.
- Authentication credential stored as environment variable, not in `forge.yaml`,
