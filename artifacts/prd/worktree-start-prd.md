...
`worktree-start` is a Forge git skill that transitions a task from the backlog
into active development by creating an isolated git worktree. It is the
standard entry point for all code execution work in a Forge-managed repo.
...
| Task updated | `.forge/tasks/<id>.md` has `status: doing` |
...
- Must read all paths from `forge.yaml` — zero hardcoded paths.
