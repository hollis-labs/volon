...
## Purpose
**Inception** is the “system builds itself” workflow: a repeatable loop where Forge uses its own artifacts (tasks, PCC, bootstrap, skills, workflows) to evolve Forge (or any repo) with high throughput and low drift.
...
- `forge.yaml` (or `.forge/forge.yaml`)
- `.forge/bootstrap.md`
- `.forge/pcc/`
- `.forge/tasks/` and `.forge/backlog/`
- `.forge/logs/`
...
- run log entry (`.forge/logs/…`)
- bootstrap updated (`.forge/bootstrap.md`)
...
Orchestrator:
- reads `forge.yaml`
- reads `.forge/bootstrap.md` (if present)
