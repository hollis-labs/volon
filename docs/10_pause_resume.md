...
Long-running work benefits from deterministic suspension points. Instead of relying on chat memory, Forge pauses by **writing state to disk** and resumes by **reading state from disk**.
...
- Pausing always updates:
  - task status + Updates
  - `.forge/pcc/tasks/<task_id>.md` — Task PCC resume capsule (L2 PCC)
...
## Bootstrap additions
Add these optional frontmatter keys to `.forge/bootstrap.md`:
