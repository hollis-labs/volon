...
Forge implements two PCC layers. Higher-numbered layers are narrower in scope and shorter-lived.

| Layer | Name | Scope | Location | Lifetime |
|---|---|---|---|---|
| L0 | Global | Project-wide invariants | `.forge/pcc/global/` | Persistent; refreshed on workflow start or git change |
| L2 | Task | Single task resume capsule | `.forge/pcc/tasks/<task_id>.md` | Per-task; written on pause, consumed on resume, retained for audit |
...
**Required files** (in `.forge/pcc/global/`):
...
**Caps:**
- Max 400 words per file (configurable via `pcc.limits.max_section_words` in `forge.yaml`)
- Max 25 files total (configurable via `pcc.limits.max_files`)

**Staleness strategy:**
- Refreshed automatically on workflow start and on detected git changes (`forge.yaml` → `pcc.refresh`)
...
**Location:** `.forge/pcc/tasks/<task_id>.md`
*Example:* `.forge/pcc/tasks/TASK-20260222-007.md`
...
**Schema:** See `.forge/templates/task-pcc-capsule.md`
...
- **No L1 iteration PCC.** Iteration state lives in `.forge/bootstrap.md`.
...
- Source docs: `docs/02_pcc.md`, `forge.yaml` (`pcc.*` keys), `docs/10_pause_resume.md`
