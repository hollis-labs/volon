...
# /forge-prompt — Prompt Generator — v0.1

## Overview

`/forge-prompt` generates a single, copy/paste-ready Forge prompt from a natural language
intent description. It eliminates manual prompt construction by:

1. Inferring the correct template from the intent
2. Reading repo context from `forge.yaml`
...
Use it to bootstrap any Forge workflow without remembering the full prompt structure.

**Aliases:** `/prompt-forge`, `/make-prompt` — identical behavior.
...
```
/forge-prompt "<intent>" [--mode inception|task|ideation|planning|patch] \
...
/forge-prompt "add layered PCC L0/L2 and wire pause/resume" --mode task

/forge-prompt "brainstorm names for the new nav component" \
  --mode ideation --constraints "lowercase, max 3 words, no abbreviations"

/forge-prompt "create a PRD and spec for sprint-based workflow" \
  --mode planning --deliverables "requirements.md, prd.md, spec.md"

/forge-prompt "apply the diff from review-fixes.patch safely"
...
| `--repo <path>` | No | Target repo path or name. Default: current directory / `forge.yaml project.name` |
...
If `--mode` is not specified, `/forge-prompt` infers the template from keywords in the intent.
...
**Use when:** starting a new iteration, installing Forge, running multiple tasks in sequence.
...
- Phase E: Backlog tasks (`.forge/tasks/TASK-*.md`)
...
### Project-level config (`forge.yaml`)

Add a `prompt_generator:` section to `forge.yaml` to set defaults for the project:
...
`/forge-prompt` is backed by:
- Skill: `plugins/prompt-forge/skills/forge-prompt/SKILL.md`
- Templates: `plugins/prompt-forge/skills/forge-prompt/templates/`
- Plugin: `plugins/prompt-forge/plugin.json`
